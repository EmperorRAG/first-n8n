#!/usr/bin/env bash
# Adds an Azure Files volume mount to an existing container app, idempotently.
# Usage: patch-volume.sh <app-name> <env-storage-name> <volume-name> <mount-path>
set -euo pipefail
APP="$1"; ENVST="$2"; VOL="$3"; MOUNT="$4"
cd "$(dirname "$0")"
. ./env.sh
...
YAML_POSIX="$WORK/app.yaml"
YAML_WIN="$(cygpath -w "$YAML_POSIX")"
az containerapp show -g "$RG" -n "$APP" -o yaml > "$YAML_POSIX"
...
az containerapp update -g "$RG" -n "$APP" --yaml "$YAML_WIN" -o none
echo "[patch] $APP: mounted $ENVST as $VOL at $MOUNT"
